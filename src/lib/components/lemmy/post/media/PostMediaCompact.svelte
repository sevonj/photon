...
<!-- 
  @component
  Thumbnail component compact and list posts.
-->

<div
  class="w-full h-full group/media {$$props.class ?? ''}"
  style={$$props.style ?? ""}
>
  <svelte:element
  ...
  >
    {#if post.thumbnail_url || isImage(post.url)}
      <img
        ...
        loading="lazy"
        class="object-cover overflow-hidden bg-slate-100 dark:bg-zinc-800 rounded-xl w-full h-full
